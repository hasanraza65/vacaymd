                ...
                <div class="middle-content container-xxl p-0">
                    ...
                    <div class="row layout-top-spacing">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-md-4">
                                    <button onclick="changeData('assigned_orders','assigned-orders-btn')" class="assigned-orders-btn btn btn-outline-info btn-rounded mb-2 me-4 w-100">Assigned To Me</button>
                                </div>
                            ...
                            @include('doctor.orders.includes.filters')

                            <div class="widget-content widget-content-area br-8 latest_orders">

                                @if(isset($_GET['start_date']) || isset($_GET['end_date']))
                                Showing Result From: {{ isset($_GET['start_date']) && !empty($_GET['start_date']) ? $_GET['start_date'] : 'Start' }} Till {{ isset($_GET['end_date']) && !empty($_GET['end_date']) ? $_GET['end_date'] : 'Today' }}
                                @endif
                                <table id="" class="table dt-table-hover html5-extension" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr id="row_{{$datas->id}}">
                                            ...
                                            <td class="action_btn" onclick="window.location ='/doctor/orders/{{$datas->id}}'">{{$datas->order_num}}</td>
                                            <td class="action_btn" onclick="window.location ='/doctor/orders/{{$datas->id}}'">{{$datas->userDetail?->name}}</td>
                                ...
                                <div class="widget-content widget-content-area br-8 assigned_orders d-none">
                                    @if(isset($_GET['start_date']) || isset($_GET['end_date']))
                                    Showing Result From: {{ isset($_GET['start_date']) && !empty($_GET['start_date']) ? $_GET['start_date'] : 'Start' }} Till {{ isset($_GET['end_date']) && !empty($_GET['end_date']) ? $_GET['end_date'] : 'Today' }}
                                    @endif
                                    ...
                                    <tbody>
                                        @foreach($assigned_tome as $datas)
                                        <tr id="row_{{$datas->id}}">
                                            <td class="d-none">{{$datas->id}}</td>
                                            <td class="action_btn" onclick="window.location ='/doctor/orders/{{$datas->id}}'">{{$datas->order_num}}</td>
                                            <td class="action_btn" onclick="window.location ='/doctor/orders/{{$datas->id}}'">{{$datas->userDetail?->name}}</td>
...
<script>
    ...
    function changeData(className,btnClass){
            ...
            if(btnClass == 'new-orders-btn'){
                ...
                $('#latest_orders_fields').removeClass('d-none');

            }else if(btnClass == 'assigned-orders-btn'){

                $('.'+btnClass).removeClass('btn-outline-info');
                $('.'+btnClass).addClass('btn-info');

                $('#assigned_orders_fields').removeClass('d-none');
                ...
                $('#completed_orders_fields').removeClass('d-none');
